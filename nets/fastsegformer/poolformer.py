...
def poolformer_s12(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        checkpoint = torch.hub.load_state_dict_from_url(url=url, map_location="cpu", check_hash=True, model_dir='./checkpoints/pretrained_weights')
...
def poolformer_s12_feat(pretrained=False, **kwargs):
    """
    PoolFormer-S12-without-fc model.
    """
    layers = [2, 2, 6, 2]
    embed_dims = [64, 128, 320, 512]
    mlp_ratios = [4, 4, 4, 4]
...
def poolformer_s24_feat(pretrained=False, **kwargs):
    """
    PoolFormer-S24-without-fc model.
    """
    layers = [4, 4, 12, 4]
    embed_dims = [64, 128, 320, 512]
    mlp_ratios = [4, 4, 4, 4]
...
def poolformer_s36_feat(pretrained=False, **kwargs):
    """
    PoolFormer-S36-without-fc model.
    """
    layers = [6, 6, 18, 6]
    embed_dims = [64, 128, 320, 512]
    mlp_ratios = [4, 4, 4, 4]
...
def poolformer_m36_feat(pretrained=False, **kwargs):
    """
    PoolFormer-M36-without-fc model.
    """
    layers = [6, 6, 18, 6]
    embed_dims = [96, 192, 384, 768]
    mlp_ratios = [4, 4, 4, 4]
...
def poolformer_m48_feat(pretrained=False, **kwargs):
    """
    PoolFormer-M48 model, Params: 73M
    """
    layers = [8, 8, 24, 8]
